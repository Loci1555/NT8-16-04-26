	...
	public class DeltaBox : Indicator
	{
		...
		protected override void OnStateChange()
		{
			if (State == State.SetDefaults)
			{
				...
				GradientScale		= 1.0;
				DeadZone			= 0.15;
				StrongBuyColor	= MkBrush(0x22, 0x87, 0xC9);
				StrongSellColor	= MkBrush(0xFF, 0x42, 0x7E);
				NeutralColor	= MkBrush(0x21, 0x21, 0x21);
				...
				AnomLookback		= 10;
				AnomStrength		= 1.5;
				AnomRobustStats		= false;
				AnomCooldown		= 0;
				AnomBuyColor		= MkBrush(0x1F, 0x82, 0xC4);
				AnomSellColor		= MkBrush(0xC9, 0x3C, 0x75);
				AnomOpacity			= 55;
				AnomTier1Radius		= 6;
				AnomTier2Radius		= 9;
				AnomTier3Radius		= 13;
				...
				AnomPanelBuyColor	= MkBrush(0x22, 0x96, 0xE3);
				AnomPanelSellColor	= MkBrush(0xD6, 0x49, 0x81);
			...
			NinjaTrader.NinjaScript.BarsTypes.VolumetricBarsType vbt)
		{
			...
			double centerBuy, centerSell, spreadBuy, spreadSell;

			if (AnomRobustStats)
			{
				// ── Median + MAD ──
				double[] buyArr  = new double[AnomLookback];
				double[] sellArr = new double[AnomLookback];
				for (int i = 0; i < AnomLookback; i++)
				{
					buyArr[i]  = anomBuyVol[i + 1];
					sellArr[i] = anomSellVol[i + 1];
				}
				Array.Sort(buyArr);
				Array.Sort(sellArr);

				int mid = AnomLookback / 2;
				double medBuy  = (AnomLookback % 2 == 1) ? buyArr[mid]
					: (buyArr[mid - 1] + buyArr[mid]) / 2.0;
				double medSell = (AnomLookback % 2 == 1) ? sellArr[mid]
					: (sellArr[mid - 1] + sellArr[mid]) / 2.0;

				double[] devBuy  = new double[AnomLookback];
				double[] devSell = new double[AnomLookback];
				for (int i = 0; i < AnomLookback; i++)
				{
					devBuy[i]  = Math.Abs(buyArr[i]  - medBuy);
					devSell[i] = Math.Abs(sellArr[i] - medSell);
				}
				Array.Sort(devBuy);
				Array.Sort(devSell);

				double madBuy  = (AnomLookback % 2 == 1) ? devBuy[mid]
					: (devBuy[mid - 1] + devBuy[mid]) / 2.0;
				double madSell = (AnomLookback % 2 == 1) ? devSell[mid]
					: (devSell[mid - 1] + devSell[mid]) / 2.0;
			...
			else
			{
				...
				centerBuy  = avgBuy;
				centerSell = avgSell;
				spreadBuy  = Math.Sqrt(sqBuy  / AnomLookback);
				spreadSell = Math.Sqrt(sqSell / AnomLookback);
			}

			double gap = AnomStrength * 0.5;
			double threshBuy1 = centerBuy  + spreadBuy  * AnomStrength;
			double threshBuy2 = centerBuy  + spreadBuy  * (AnomStrength + gap);
			double threshBuy3 = centerBuy  + spreadBuy  * (AnomStrength + gap * 2.0);

			double threshSell1 = centerSell + spreadSell * AnomStrength;
			double threshSell2 = centerSell + spreadSell * (AnomStrength + gap);
			double threshSell3 = centerSell + spreadSell * (AnomStrength + gap * 2.0);
		...
		[Display(Name = "Robust Stats",
			Description = "Median+MAD замість Mean+Std. Стійкий до спайків при короткому lookback",
			GroupName = "5. Volume Anomaly", Order = 3)]
		public bool AnomRobustStats { get; set; }
		...
		[Display(Name = "Шрифт панелі (pt)",
			GroupName = "5. Volume Anomaly", Order = 16)]
