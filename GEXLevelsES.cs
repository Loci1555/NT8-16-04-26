...
namespace NinjaTrader.NinjaScript.Indicators.Loci
{
	public class GEXLevelsES : Indicator
	{
		...
		private SharpDX.Direct2D1.Brush		dxBrush5;
		...
		#region Параметри — SPX 5s

		[NinjaScriptProperty]
		[Display(Name = "Показати", Order = 0, GroupName = "5. SPX 5s")]
		public bool Show5 { get; set; }

		[XmlIgnore]
		[Display(Name = "Колір", Order = 1, GroupName = "5. SPX 5s")]
		public Brush Color5 { get; set; }
		[Browsable(false)]
		public string Color5Serialize
		{
			get { return Serialize.BrushToString(Color5); }
			set { Color5 = Serialize.StringToBrush(value); }
		}

		[NinjaScriptProperty]
		[Display(Name = "Тип лінії", Order = 2, GroupName = "5. SPX 5s")]
		public DashStyleHelper DashStyle5 { get; set; }

		[NinjaScriptProperty]
		[Range(1, 5)]
		[Display(Name = "Товщина", Order = 3, GroupName = "5. SPX 5s")]
		...
		[XmlIgnore]
		[Display(Name = "Колір", Order = 0, GroupName = "6. Лейбли")]
		...
		[NinjaScriptProperty]
		[Range(6, 24)]
		[Display(Name = "Розмір шрифту", Order = 1, GroupName = "6. Лейбли")]
		...
		protected override void OnStateChange()
		{
			if (State == State.SetDefaults)
			{
				...
				Color25.Freeze();
				DashStyle25			= DashStyleHelper.Dash;
				Width25				= 1;

				// 5s
				Show5				= true;
				Color5 = new SolidColorBrush(System.Windows.Media.Color.FromArgb(0xFF, 0x3F, 0x3F, 0x3F));
				Color5.Freeze();
				DashStyle5			= DashStyleHelper.Dot;
				Width5				= 1;
		...
		protected override void OnRender(ChartControl chartControl, ChartScale chartScale)
		{
			...
			foreach (var level in levels)
			{
				...
				if (level.Tier == 3 && !Show5)  continue;
				...
				switch (level.Tier)
				{
					...
					case 2:  brush = dxBrush25;  width = Width25;  dashHelper = DashStyle25;  break;
					default: brush = dxBrush5;   width = Width5;   dashHelper = DashStyle5;   break;
				}
		...
		private void RecalculateLevels()
		{
			...
			for (int strike = spxCenter - RangePoints; strike <= spxCenter + RangePoints; strike += 5)
			{
				int tier;
				if (strike % 100 == 0)		tier = 0;
				else if (strike % 50 == 0)	tier = 1;
				else if (strike % 25 == 0)	tier = 2;
				else				tier = 3;
		...
		private void EnsureResources(ChartControl chartControl)
		{
			...
			if (dxBrush5 == null || dxBrush5.IsDisposed)
				dxBrush5 = Color5.ToDxBrush(RenderTarget);
		...
		private void DisposeResources()
		{
			...
			if (dxBrush5 != null)	  { dxBrush5.Dispose();     dxBrush5 = null; }
